...
class GreenResultsController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        for i in 0 ..< samples.count-1 {
            if(i != 2){
                greens.append(Double(samples[i].size.width))
            }
        }
        updateGraph()
    }
    ...
    func updateGraph(){
        ...
        let linRegress = LinearRegression(x: conc, greens)
        
        equation.text = String(format: "y = %.4f x + %.2f", linRegress.slope, linRegress.intercept)
        
        let intensity = Double(samples[NO_OF_RECTS-1].size.width)
        let concentration = (intensity - linRegress.intercept)/linRegress.slope
        let qcIntensity = Double(samples[2].size.width)
        ...
        for i in 0 ..< greens.count{
            let point = ChartDataEntry(x: conc[i], y: greens[i])
