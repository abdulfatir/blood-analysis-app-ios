...
class BlueResultsController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        for i in 0 ..< samples.count-1 {
            if(i != 2){
                blues.append(Double(samples[i].size.height))
            }
        }
        updateGraph()
    }
    ...
    func updateGraph(){
        let linRegress = LinearRegression(x: conc, blues)
        
        equation.text = String(format: "y = %.4f x + %.2f", linRegress.slope, linRegress.intercept)
        
        let intensity = Double(samples[NO_OF_RECTS-1].size.height)
        let concentration = (intensity - linRegress.intercept)/linRegress.slope
        let qcIntensity = Double(samples[2].size.height)
        ...
        for i in 0 ..< blues.count{
            let point = ChartDataEntry(x: conc[i], y: blues[i])
